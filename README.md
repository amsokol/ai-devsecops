...
Monorepo for the AI DevSecOps **agent** (runner) and its **knowledge library**. The agent reviews
proposed changes and maintains a repository's default branch. Judgement lives in prose under
...
| [`agent/`](agent/) | CLI, planner, tools, backends, publish |
| [`DESIGN.md`](DESIGN.md) | one design for both halves |
...
```bash
uv sync
uv run agent review  --repo . --change 12 --base main [--publish]
...
overrides that path for tests only. One product version covers runner and judgement; there is no
...
| Path | Meaning |
| --- | --- |
| `--library` | optional override of the bundled knowledge tree |
