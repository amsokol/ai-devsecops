...
async def run_attempts[T](
    ...
    *,
    ...
    for number in range(1, MAX_ATTEMPTS + 1):
        ...
        if attempt.strayed and task.role is Role.FIXER:
            # A fixer that edited the checkout believes it made a change that is not on the branch.
            # Reading its report would put a claim of a fix on a tree that has none.
        ...
        try:
            ...
            if attempt.strayed and task.role is not Role.FIXER:
                # Analyst wrote outside the worktree *and* produced no usable result: same retry as
                # a plain invalid file, with the stray reason first so the next prompt names both.
                attempt.rejected = rejection = attempted.rejected = (
                    f"{refusal(attempt.strayed)}; also {error}"
                )
            else:
        ...
        if attempt.strayed and task.role is not Role.FIXER:
            # Analysis does not ship a branch: undoing the stray writes is enough. Accepting the
            # valid result avoids a full second session (demo2 BSR: ~4M tokens retried for the same
            # findings after `buf generate` wrote into the checkout under sandbox:false).
