"""Loading the knowledge library: identity, index, slices, content digest.
...
class Identity:
    """What the library says about its result contract, from `library.yaml`."""
    ...
    def read(cls, root: Path) -> Self:
        ...
        if "contract_version" not in raw:
            raise ConfigError(f"{path}: missing contract_version")
        ...
        return cls(contract_version=contract)

    def check_compatible(self) -> None:
        """Refuse an unknown result contract rather than failing mid-run on result.json."""
...
class Library:
    ...
    def digest(self) -> str:
        ...
        Recorded on the run for audit. Not compared to a pin — the agent release *is* the knowledge
        release. Deliberately not a digest of the whole directory (README, scripts, templates).
        """
        entries = [
