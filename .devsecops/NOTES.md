...
- Knowledge ships with the agent release. There is no separate library pin; weakening startup
  contract checks is still a defect.

## Cautions

- Exit codes are an interface: CI acts on them. Changing one is a breaking change.
- Verification for `python-uv` includes markdownlint over `**/*.md` (same as CI). A fix that leaves
  Markdown lint failing is not shippable, including knowledge and DESIGN edits on the same surface.
