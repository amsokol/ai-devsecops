# Design of AI DevSecOps

Status: draft. This document describes the **monorepo** `ai-devsecops`: the knowledge library
(`knowledge/`) and the agent runner (`agent/` package, CLI, tools) that executes it under a product
overlay.

This design document is English, as are the code, knowledge artefacts, configuration, schemas and
README.

Judgement lives in prose under `knowledge/` (`CONTRACT.md` is the exchange contract). Execution —
tools, budgets, SCM actions, digests — lives in the runner. The split of *concerns* remains; the
split of *repositories* does not.

## 0. Repository layout

```text
/
├── DESIGN.md                 # this document (knowledge + runner)
├── README.md, CHANGELOG.md, LICENSE
├── pyproject.toml, uv.lock   # runner package
├── agent/                    # Python package: CLI, planner, tools, backends
├── tests/, scripts/
├── agent/config/             # run configuration (limits, ceiling, backends)
├── knowledge/                # judgement library (prose + INDEX + contract identity)
│   ├── CONTRACT.md
│   ├── INDEX.md, library.yaml   # library.yaml: contract_version only
│   ├── playbooks/, capabilities/, policy/, ecosystems/, …
│   └── scripts/library.py
├── .devsecops/               # dogfood overlay for this repo
└── .github/workflows/        # runner CI + knowledge checks

```

Knowledge ships inside the agent wheel. Products pin one agent tag (`@vX.Y.Z`); there is no separate
library pin or digest gate.

---

## Part A — Knowledge library

The library is the agent's only source of **judgement**: what counts as a problem, what to look at,
in what order to act, what to be careful about, when to block, and when only to warn.

The library **does not contain** code or executable product logic, data schemas, run configuration
(models, backends, budgets), or product values (quarantine length, verification commands, enabled
ecosystems). Those belong to the runner or the product overlay. Knowledge stays readable prose;
precision and reproducibility stay code.

### A.1. Purpose and boundaries

The knowledge library is the agent's only source of **judgement**: what counts as a problem, what to
look at, in what order to act, what to be careful about, when to block, and when only to warn.

The library **does not contain**:

- code or executable scripts;
- data schemas (JSON Schema and the like);
- run configuration (which model, which backend, which timeouts and budgets);
- product values (quarantine length, verification commands, enabled ecosystems).

That split is the library's main architectural decision. It follows from the fact that knowledge is
edited by people and must stay readable, while everything that needs precision and reproducibility
must be code.

### A.2. Principles

1. **Prose is the canonical carrier of knowledge.** A skill's body is ordinary Markdown, written for
   a human and a model at once. No machine settings inside the body.
2. **A table of contents, not configuration.** Every document has a minimal header of four fields.
   Its job is to give the agent a cheap menu so it loads only what it needs. That is an index, not a
   description of behaviour.
3. **Progressive disclosure.** A subagent receives a slice of the library for its task, not the whole
   catalogue. Saving tokens matters here not only for cost: extra context lowers accuracy.
4. **One claim lives in one place.** A shared procedure is written once; ecosystem documents keep only
   the differences. Duplicated prose is a source of drift.
5. **Knowledge does not invent tools.** A skill describes which data source to use and how to read
   the result, but does not replace the tool's implementation. The set of available tools is fixed by
   the agent contract.
6. **Symmetry of the unknown.** For every check, a skill must describe not only "found" and "not
   found", but also "could not check". Silent "all clear" when data is unavailable is forbidden.
7. **Structure does not create empty files.** There is no mandatory matrix of "ecosystem × five
   topics". A document exists when it has content.
8. **Separating fact acquisition from decision.** See section 4.

### A.3. Four layers of the system

The library is one of four layers. The boundaries between them should be visible from any document.

| Layer | Where it lives | What it is | Examples |
| --- | --- | --- | --- |
| Knowledge | this library | prose for humans and models | severity criteria, review order, cautions on major bumps |
| Tools | the runner package | deterministic code | running scanners, HTTP to registries, quarantine arithmetic, git and SCM API |
| Run configuration | the runner package | a small YAML | trigger → task set, utility and host ceiling, adapter settings |
| Product overlay | the product repository | YAML plus short prose | models and limits per kind of run, quarantine N, verification, ecosystems |

Rule of thumb when in doubt: if a claim needs precision and reproducibility — it is a tool; if it
needs context and judgement — it is knowledge; if it differs from product to product — it is the
overlay.

### A.4. Two phases of a run and how knowledge reflects them

The agent works in two phases, and the library's structure follows that split.

**Evidence acquisition phase.** Any way of obtaining a fact is allowed: running a scanner, calling a
registry API, invoking a CLI, reading release notes, parsing a page. Ecosystems such as Buf BSR or
the Bazel Central Registry have no ready-made audit tools, so some evidence is gathered "by hand",
through APIs and the web. Every piece of evidence is recorded uniformly: what had to be learned,
which source was used, which value was obtained, when, and by which means — a deterministic tool or
extraction by the model.

**Decision phase.** Works only over the collected evidence, by criteria from the library, and does
not touch the network at all.

Three requirements on knowledge follow:

- **The right to block depends on where the fact came from.** Reproducible evidence (scanner output,
  an API answer) may lead to a hard block. A fact the model extracted from a web page or release notes
  is filed as a remark with the link and the obtained value attached, so a person can check it in ten
  seconds. That is not a weaker gate — it is the split between "proven" and "looks like".
- **The "unknown" state is described explicitly, and it comes in two kinds.** If no tool for the fact
  exists and that is written in the ecosystem profile — that is a documented limit: the agent reports
  "could not check" and does not block the merge, otherwise an ecosystem without a scanner would block
  everything forever, and such a gate gets switched off. If something that should have worked failed
  — the scanner crashed, the host did not answer, the page changed shape, the budget ran out — that is
  a **failure**, and the run ends with an inconclusive result that does not allow the merge. Without
  that distinction, the right to merge (the check's exit code) is disabled by anyone who can break a
  scanner or exhaust a budget: absence of a result must not be sold as a result.
- **Closing findings needs confirmation, not absence.** A finding closes when the task finished with
  "checked, clean", not when it simply went unmentioned. Otherwise the first scanner failure would
  close every known problem as solved — exactly when the tools are least to be trusted.
- **External text is untrusted data.** Registry pages, release descriptions and changelogs may contain
  injected instructions. Knowledge must require that such content is fed to the model as isolated
  data, and that instructions inside it are ignored.

### A.5. Catalogue layout

```text
knowledge/
├── README.md                  # what this is, how to attach it, how to read it
├── (design lives at repo root DESIGN.md)
├── CONTRACT.md                # contract between library and agent (English)
├── INDEX.md                   # generated table of contents; the agent's entry point
├── library.yaml               # contract_version only (product version is pyproject.toml)
├── playbooks/
│   ├── pr-review.md           # what to do when reviewing a change
│   └── maintain.md            # what to do when maintaining the default branch
├── capabilities/
│   ├── code-quality.md
│   ├── code-vuln.md
│   ├── deps-outdated.md
│   └── deps-vuln.md
├── policy/
│   ├── verdicts.md            # finding classes, severity, rights to block
│   ├── unknowns.md            # behaviour when data is unavailable
│   ├── quarantine.md          # meaning of quarantine; value of N is in the overlay
│   ├── holds.md               # pin holds and human unlocks
│   ├── grouping.md            # how to split changes into reviewable PRs
│   ├── bundles.md             # related dependencies updated together
│   └── verification.md        # which surfaces to check after an edit
├── evidence/
│   └── acquisition.md         # shared procedure for gathering and recording evidence
├── ecosystems/
│   ├── python-uv.md
│   ├── python-pip-compile.md
│   ├── npm.md
│   ├── cargo.md
│   ├── go-modules.md
│   ├── github-actions.md
...
│   └── bsr.md
├── scm/
│   └── github.md              # the only supported SCM
├── overlay/
│   ├── README.md              # what a product must define for itself
│   └── templates/             # templates for agent.yaml (values) and NOTES.md (prose)
├── scripts/
│   └── library.py             # contract checks and INDEX.md generation
├── (CI lives at repo root .github/workflows)
└── fixtures/                  # golden sets for quality evaluation (see section 12)
```

Differences from the previous library that need explaining:

- **One ecosystem — one document.** The previous five files per ecosystem were structurally almost
  identical and differed in data, not judgement; part of their content moves into the evidence
  acquisition phase. Turning a document into a directory `ecosystems/<id>/` is allowed, but only when
  the file has actually grown large — not in advance.
- **`policy/verdicts.md` appeared.** The single place that describes finding classes, the severity
  scale, and rights to block depending on evidence origin.
- **`policy/unknowns.md` appeared.** Incomplete coverage used to be marked "MVP" — documentation for
  humans that did not affect agent behaviour. Now it is described behaviour.
- **`evidence/acquisition.md` appeared.** A shared procedure so it is not repeated in every ecosystem.
- **Scenarios are named playbooks.** The old split into `scenarios/`, `gate/` and `maintain/`
  scattered one idea across three places.
- **GitLab is out.** The old `scm/gitlab.md` was knowingly incomplete and created a false impression
  of support. The library describes only GitHub; a new SCM is added as a separate document with
  `kind: scm` when a real consumer appears.

### A.6. Knowledge document format

Every document consists of a minimal header and a prose body.
...
- `id` — a stable identifier matching the path without the extension. Used in links and in agent
  reports.
- `kind` — one of `playbook`, `capability`, `ecosystem`, `policy`, `scm`, `overlay`.
- `summary` — one line. The agent decides from it whether to load the body, so it must describe the
  content, not advertise it.
- `applies_to` — an optional list of file markers or ecosystems that helps select the document from a
  diff.

No further fields. The moment thresholds, model versions or conditions appear in the header — that is
already configuration, and its place is in the runner package (`agent/`).

Exception to the rule "`id` matches the path": templates in `overlay/templates/`. Those are artefacts
the product copies to itself, so a template's `id` matches the future path in the product, not the
path in the library. Templates do not appear in `INDEX.md`. The header format applies only to
knowledge documents: `agent.yaml` is data; it has a schema on the agent side, not a header.

The body is written in the imperative, addressing the agent. For each `kind` a set of sections is
fixed — that is a contract of form, not of data, and CI checks it:

| `kind` | Required sections |
| --- | --- |
| `playbook` | Trigger, Tasks, Evidence needed, Aggregation, Verdict and actions, Degradation |
| `capability` | What to look for, Evidence needed, Judgement criteria, Severity, Fix policy, False positives |
| `ecosystem` | Capability profile, Requirements, Detect, Evidence recipes, Update procedure, Cautions |
| `policy` | free form |
| `scm` | Capabilities, Procedures, Permissions |

### A.7. Index and selective loading

`INDEX.md` is generated from document headers and contains a table of "id, kind, summary,
applies_to". It is the only file the agent reads whole and always. After that it loads only the bodies
selected by the playbook, by the ecosystems enabled in the overlay, and by the files touched in the
diff.

Currency of `INDEX.md` is checked in CI: divergence from the headers is a build error.

Expected effect: a dependency-vulnerability subagent on a Python project receives a playbook, one
capability, one ecosystem and two policy documents — a few kilobytes instead of the whole catalogue.

### A.8. Ecosystem capability profile

The key part of an ecosystem document is a table that honestly describes what is obtained and how. It
is written in prose but structured, because from it the agent knows what to expect.

| Required fact | Means | Source | Reliability |
| --- | --- | --- | --- |
| current pins | tool | manifest and lock files | reproducible |
| available versions | API | ecosystem registry | reproducible |
| version publish date | API | ecosystem registry | reproducible |
| vulnerabilities | tool | ecosystem scanner | reproducible |

Values in the "Means" column: `tool`, `api`, `web`, `none`. The value `none` is not a documentation
gap — it is an instruction: the agent must return "could not check" for that fact. For ecosystems
without ready-made scanners (BSR, Bazel) a large share of rows will be `web` or `none`, and that is a
normal, described state, not unfinished work.

Beside the profile, the ecosystem document declares in the `Requirements` section which utilities and
which hosts its procedures need. That is a request, not a grant: the agent satisfies it only within
its own ceiling — the set of utilities and hosts it is willing to use at all. A line added to a
knowledge document cannot widen the agent's rights. Something requested but outside the ceiling is
rejected at startup with a clear message, rather than appearing as a mysterious failure mid-run.

That arrangement keeps the library's main property: adding an ecosystem remains a Markdown edit, and
environment requirements live next to the knowledge that creates them — while the final decision
stays with the agent.

Evidence with means `web` is cached under the key "ecosystem, package, version": a version's publish
date does not change, and the version list changes slowly. Caching removes the main cost and the main
instability exactly where both are highest.

### A.9. Contract with the agent

`CONTRACT.md` (in English) fixes what a skill author may rely on:

- the list of available tools and their semantics: reading and searching files, running a utility from
  an allowlist, an HTTP request to an allowlisted host, SCM API calls, file edits and git operations
  for writing tasks;
- fields of an evidence record, including means of acquisition and a link to the source;
- fields of a finding, including a stable key for idempotency, class, severity and rationale;
- task completion states: result obtained, no findings, could not check, budget exhausted;
- how the result is returned: the subagent writes the result to an artefact file at a fixed path, and
  the agent validates it after the session ends.

The last point matters: agent SDKs return a stream of messages, and parsing the final text is a
source of fragility. An artefact file makes the protocol independent of how chatty the model turned
out to be.

Schemas for these structures live in the agent's code, not in the library. Here — only their
description for the skill author.

### A.10. Product overlay

The overlay holds two things of different nature, split across files because they are used
differently:

- **Values** (`agent.yaml`): what review does and what maintenance does — models, spending limits, and
  for maintenance also a ceiling on how much it may write to the tracker — plus quarantine length,
  enabled ecosystems, hotspots, local exceptions with rationale, verification commands per surface.
  These are structured data: the agent computes on them and executes them.
- **Knowledge about the product** (`NOTES.md`, shaped as an ordinary document with `kind: overlay`):
  architectural invariants the agent must account for in review, and product-specific cautions. The
  shared library cannot hold that knowledge, because it is not reused.

Values are deliberately not prose. A quarantine length or a command extracted from a sentence
disappears the moment a person rephrases that sentence, and nothing fails: the run continues with a
different quarantine window or an unverified fix. A silent change of meaning is worse than a loud
parse error, so `agent.yaml` is checked against a schema, and an unknown key is a startup error.

The overlay is organised by kind of run: the agent does two things — reviews a change and maintains
the default branch — and the `review:` and `maintenance:` sections describe each entirely. Both are
required, and the models and limits inside them are required too; that is not a matter of
configuration style. A product outlives any provider: a subscription ends, a new adapter appears, an
important project decides its reviews are worth an expensive model. A default inside the agent would
turn every such case into a fork of the agent, and the agent into the one deciding how somebody else's
money is spent. Provider keys come from the environment and never from the overlay: the overlay lives
in version control.

Two constraints on the overlay.

First: `NOTES.md` lands in every subagent's context, so its size is hard-limited. This is the place
for invariants and cautions, not project documentation. Exceeding the limit is a warning at run
start, because bloated notes crowd out useful context and make every run more expensive.

Second: the overlay sets values and narrows scope, but does not override criteria. In particular it
cannot weaken rules of the "security" class. The constraint is described in `overlay/README.md` and
checked by the agent at startup.

### A.11. Versioning and delivery

The **product version** is the agent release in `pyproject.toml`. Knowledge and runner ship in one
tag. SemVer in the `0.x` range: middle number on a breaking change (renamed `id`, removed document,
changed required sections, contract change), last number on a compatible one. Changes are recorded
in the root `CHANGELOG.md`.

Products install or pin that one agent tag. There is no separate knowledge artefact, digest pin, or
`min_agent_version`. `knowledge/library.yaml` carries only `contract_version` so the runner can refuse
an unknown result protocol if someone overrides `--library`.

### A.12. Quality and checks

Library CI runs markdownlint and `scripts/library.py check`, which verifies:

- headers match the format, `id` matches the path, `id`s are unique, `summary` is non-empty;
- required sections exist for each `kind`;
- all relative links resolve;
- `INDEX.md` is current (generated by the same command with the `index` argument);
- the newest root `CHANGELOG.md` section matches the product version in `pyproject.toml`;
- there are no documents unreachable from any playbook. Ecosystems and the overlay are excluded from
  that check: they are selected by identifier from the overlay, not by link, so link reachability
  says nothing about them.

Separately — `fixtures/`: a set of fixed repository snapshots with golden findings. The runs
themselves are executed by the runner package, but the goldens live here because they change with
the knowledge. That is the only way to answer "which model is better for this task" by measurement,
not taste, and it also guards against regressions when prose is edited.

### A.13. Mapping from the previous library

| Was (`ai-devsecops-skills`) | Is |
| --- | --- |
| `policy/entry.md` | `INDEX.md` plus `README.md` |
| `scenarios/gate.md`, `gate/change-review.md` | `playbooks/pr-review.md` |
| `scenarios/maintain.md`, `maintain/*.md` | `playbooks/maintain.md`, partly `policy/*` |
| `policy/signals.md` | `CONTRACT.md` (completion states) and `policy/verdicts.md` |
| `capabilities/deps-policy.md` | `capabilities/deps-outdated.md` |
| `ecosystems/<id>/{detect,update,publish-time,advisories,caution}.md` | `ecosystems/<id>.md` plus `evidence/acquisition.md` |
| "MVP" marks in prose | `none` column in the capability profile and `policy/unknowns.md` |

The previous pair of repositories is frozen at their tags: products pointing at
`ai-devsecops-cursor@v0.3.15` and submodule `v0.1.16` keep working until they move. Products are
migrated one by one, and during the transition a product must have exactly one blocking check status,
otherwise the gates start arguing with each other.

### A.14. Accepted decisions

- Knowledge ships inside the agent release; products pin one agent tag (section 11).
- Knowledge about the product itself lives in the overlay as a separate document with `kind: overlay`,
  not in the shared library (section 10).
- The overlay is split by nature of content: values in `agent.yaml` with a schema, prose for the model
  in `NOTES.md`. Values are not extracted from prose, because a rephrased sentence changes meaning
  silently (section 10).
- GitLab is out; the library describes only GitHub (section 5).
- Required utilities and hosts are declared by the ecosystem document in `Requirements`, and the agent
  satisfies the request within its own ceiling (section 8).
- A run has three outcomes: allowed, blocked, and inconclusive. Failure of a required check yields an
  inconclusive result that does not allow the merge and asserts nothing about the code (section 4).
- What exactly blocks is defined by the "class × severity" table in `policy/verdicts.md`, not decided
  anew each run. The right to block is further limited by evidence origin.
- A heuristic publish date is used asymmetrically: it may delay a candidate, but may allow it only when
  the version is unambiguously older than the window. Otherwise an ecosystem without proper timestamps
  either freezes forever or accepts versions on a guessed date.
- `capabilities/code-quality.md` covers correctness and maintainability risks: unhandled errors, lost
  execution branches, resource leaks, wrong API use, concurrency issues, behaviour at input
  boundaries. Style — line length, import order, naming conventions, idiom preferences — is excluded
  from the capability. Model opinion on those questions is non-deterministic, turns into noise in a
  blocking gate so that substantive remarks stop being read, and linters and formatters give the same
  result exactly and identically every run. If a product needs a style check, that is a linter in
  verification, not a model judgement.

---

## Part B — Agent runner

The agent is the execution layer. It contains no judgement of its own: severity criteria, quarantine
rules, ecosystem procedures and product values live in `knowledge/` and the overlay. The runner owns
what must be exact and reproducible: turning triggers into parallel tasks, deterministic tools,
verdicts and platform actions, budgets, and an audit trail.

### B.1. Purpose and boundaries
...
### B.2. What changed relative to the previous agent
...
is known teaches the team to switch the agent off. The `ai agent` label is not proof of authorship —
anyone can add it — so reconciliation trusts only the marker (legacy label `agent` is still read so
older tickets are not duplicated). The title is built from the parts that identify the problem and
does not contain what drifts (versions, advisory identifiers): otherwise a saved search stops
matching. Severity is shown as an emoji prefix on the title; code findings lead with
`code quality` / `code vulnerability` plus a stable summary, not the raw capability id or path.
The number of new issues per run is limited by the overlay; the rest are deferred to the next run
and named in the record, but not dropped and not merged into one issue.
...
`this repository (runner)` with a `users.noreply` address: platforms associate commits with accounts by
...
### B.11. Compatibility and versions

One product version covers runner and knowledge. The agent declares the set of library
`contract_version` values it can read; `knowledge/library.yaml` names the contract the bundled prose
implements. A contract mismatch (for example via `--library` override) is a startup error.

A content digest of identity, index and bodies may be recorded on the run for audit. It is not
compared to a pin file — the agent tag *is* the knowledge release. Products pin `@vX.Y.Z` only.

### B.12. Quality evaluation

Without measurements, choosing a model for a task remains taste. Therefore an eval harness lives in
the runner package, and the goldens live in the library's `fixtures/`, because they change with the
knowledge.
...
│   ├── library/               # load bundled knowledge, slice by INDEX
...
here marked **Done in agent X.Y.Z**; the CHANGELOG carries the release note.
...
is not mistaken for a full sweep.
...
**Done in agent 0.4.0 (all listed ecosystems):** `list_declared_pins(ecosystem=…)` censuses cargo,
npm, python-uv, python-pip-compile, go-modules, bazel, bsr, and github-actions; the incomplete-sweep
gate is `incomplete_pin_sweep`. `list_action_pins` remains an alias for github-actions.
...
**Done in agent 0.4.0:** `closed_issues` + `reopen_issue` on the platform port; tracking reopens the
newest closed agent issue for that key (does not consume `max_new_issues`), notes that the finding
is back, and updates the body. Closing notes no longer promise a new ticket on return.
...
**Also in agent 0.6.0:** reclaim detaches any worktrees still sitting on the abandoned tip before
`branch -D`. An interrupted maintain that left a worktree holding `agent/routine/…` used to make
every later reclaim fail with "branch already exists", so the next run deferred forever. Unsettled
fix worktrees are discarded if `apply` is cancelled mid-run.
...
**Done in agent 0.4.0:** the new-issue ceiling counts distinct subjects (or bundle ids), not
advisory rows. Nine advisories on one pin no longer consume nine slots; unrelated pins still each
count once. **Also:** vulnerable findings are keyed by `kind` (`…:vulnerable`), not by advisory id;
`merge` folds every advisory on one pin into one finding before publish, and tracking migrates
legacy per-advisory issues onto that pin ticket. Analysts may still emit one result row per
advisory; the tracker sees one conversation.
...
**Also seen on full maintain (demo2, agent 0.2.4):** ~20.6M tokens before any fixer — BSR alone ~7M
(false null `cleared_pin_target` for plugins → manual `buf` crawl + full analyst retry after stray
`buf generate` writes); bazel/go/python/cargo still second-guessed `target=null` with registry CLIs.
**Done in agent 0.2.5 (library 0.5.7), first cut:** BSR module+plugin label list; salvage valid
analyst results after stray checkout writes; knowledge forbids registry second-guess after
`cleared_pin_target`.
**Done in agent 0.5.0:** after empty BSR plugin labels, `cleared_pin_target` reads the
`bufbuild/plugins` catalog (commit date, then `source_url` GitHub Release) before any
`owner/name` GitHub fallback — protoc plugins such as buffa no longer fall through to a false
`unknown_age` / null tip.
**Done in agent 0.4.0 (remainder):** tool stdout/file delivery capped (`run_command` /
`read_file` share the model payload refusal path; command soft-truncate 8k); run-level memo of
`cleared_pin_target` answers on the shared session; optional overlay `fixer_token_reserve` so
analysts cannot spend the whole `tokens_per_run` before fixers; optional `sweeper` role for maintain
`deps-outdated` (falls back to `analyst` when unbound).
**Cost wave 1 (still 0.4.0, after dogfood `20260731T185621Z-b78cdce8` ~25M tokens):** (1) sweeper
prompt knowledge is a slim allowlist — capability + ecosystem + quarantine/verdicts/bundles/
grouping/acquisition, no maintain playbook; (2) planner skips `deps-vuln@…` when the ecosystem
capability profile marks advisories `method none` (no session to rediscover a documented gap);
(3) overlay template binds `sweeper` to a cheaper model by default.
**Done (Variant A, still 0.4.0):** outdated prep+judge — the runner acquires the pin census and
every `current-cleared` answer, writes a pack, seeds evidence, and hides registry tools; the sweeper
only judges from the pack and emits findings. **Variant B** (runner emits routine findings without
a judge session) remains open.
**Done (Variant A for deps-vuln, still 0.4.0):** vuln prep — the runner runs the ecosystem scanner
(`pip-audit` / `govulncheck` / `cargo audit` / `npm audit`), writes a pack, seeds `advisories`
evidence, and hides `run_command`/`fetch`; the vuln role judges from the pack. Web/`none`
ecosystems keep the tools-first path. Code-vuln prep remains open.
...
without "fixing" it by adopting a newer version that is also inside the window.
**Done in agent 0.4.0 (contract 3):** `cleared_pin_target` records `current-cleared` evidence; the
deps-outdated parse path fails when a **tool-written** value is not true and no matching finding
carries `forbidden_state` (kind quarantine, floating, or vulnerable). `record_fact` refuses that
question — the runner owns the arithmetic. CONTRACT kind order matches the kind table
(current/resolved uncleared → quarantine). All listed ecosystems.
**Verified on demo2 (local 0.4.0, maintain `--only deps-outdated@github-actions`):** seeded
`actions/checkout@v7.0.1` with quarantine 14d → issue
[#29](https://github.com/amsokol/ai-devsecops-demo2/issues/29) (`kind: quarantine`,
`forbidden_state`); also raised [#30](https://github.com/amsokol/ai-devsecops-demo2/issues/30)
(`setup-python` tip in window). First dogfood passes failed when the model re-recorded the whole
tool payload as `current-cleared`; the gate now ignores non-tool facts for that question.
**Ownership fix (still 0.4.0, dogfood `20260731T182227Z-b78cdce8`):** the gate initially treated
run-global `current-cleared` facts as every analysis task's obligation, so one uncleared cargo pin
invalidated every sibling ecosystem and every `deps-vuln` result. Completeness gates now follow the
same ownership as the pin census: facts stay run-shared; obligations charge only
`deps-outdated@<ecosystem>`. Future completeness gates must take task ownership (capability +
ecosystem), never raw `tuple(evidence)` alone.
**Done in agent 0.5.0 (contract 5):** `current_cleared: null` is finding kind `unknown_age`
(release date unknown), not quarantine. The quarantine gate requires that kind when publish time is
null; quarantine remains only for `current_cleared: false`. Issue title phrase: **release date
unknown**.
...
**Also in agent 0.6.0:** PyPI backends strip `name==version` requirements and fall back to the
project releases RSS + per-version JSON when the full package document is truncated (false
`unknown_age` on large packages such as ruff). Floating action refs with no concrete tip (`@stable`)
are marked `float_like` so prep asks for `floating`, not `unknown_age`.
...
findings with no cleared candidate. Depends on reliable target selection (item 9).
...
**11. A coupled bundle opens one issue per member pin.** On `demo2` full maintain, the `rust-buffa`
bundle (BSR plugin `buf.build/anthropics/buffa` + cargo crate `buffa`) produced two routine outdated
findings and two issues (#23 and #24) with the same remediation text, then two refuse comments for
the same `connectrpc` constraint. Tracking is still "one issue per finding key"
(`capability:ecosystem:package:kind`), and fix grouping is still `class + subject(pin)`, so parallel
ecosystem analysts correctly emit member findings and the publish path never collapses them. The
library already says majors unlock with one issue per bundle; routine drift does not get that
treatment. Direction — tasks for the runner:

1. **Collapse before publish.** After parallel ecosystem analysts, reconcile member findings that
   share a bundle id into one tracked unit before `raise_issue` / fix scheduling — never one ticket
   per member when the comment pass (or NOTES) named a bundle.
2. **Stable bundle key.** Prefer an identity keyed by bundle id (e.g. `…:bundle:<id>:outdated` /
   vuln), not by whichever member happened to run. Partial weeks (cargo dirty, BSR clean, or the
   reverse) must update the same issue, not open a sibling or orphan the old one.
3. **Absence and close for the whole set.** The issue stays open while any member is still dirty;
   close only when every member of that bundle is clean under the existing complete×2 rules —
   absence of one member must not look like disappearance of the problem.
4. **Fix grouping by bundle.** When a finding carries a bundle id, one fix job / one branch / one
   refuse-or-propose comment for the set, matching the atomic apply already described in remediation
   prose. Keep pin-level grouping only for non-bundle subjects.
5. **Migration.** Decide what to do with already-open dual member issues (close the duplicate with
   a pointer, or rewrite markers onto the surviving bundle issue) so the next maintain does not
   thrash.

Until that lands, the agent may still *apply* a bundle as one change-set when a fixer is lucky, but
the tracker UX remains split — two issues, two refuses, one real problem.

**Done in agent 0.4.0 (contract 4):** findings may carry `bundle`; the runner collapses members,
keys by `capability:bundle:<id>:…`, groups fixes by bundle, remembers members for absence, and
closes legacy per-member issues with a pointer to the bundle ticket.
**Also in agent 0.5.0:** routine `outdated` issues only when the move can ship (bundle +
cross-bundle / constraint blockers). A cleared tip that cannot build with the rest of the tree is
report-only, not a dependency-update ticket.

**12. An audit that crashed is not the same as an audit that found nothing.** In the demo `pip-audit`
resolves both requirement sets into one environment; the clash between the runtime pin `urllib3`
1.26.4 and the dev pin 2.7.0 made resolution impossible, and the step died before it looked at a
single advisory, leaving a red build with a dependency-conflict message. Direction:
`ecosystems/python-pip-compile.md` must describe this case — consistency of the two sets as something
to check, and "the tool could not resolve dependencies" as a separate fact with its own consequence,
not as absence of vulnerabilities.

**Done in agent 0.4.0:** `ecosystems/python-pip-compile.md` treats a resolver conflict as
`unverified` / a consistency finding, never as a clean vuln sweep.

**13. Renaming the tracker label broke publish.** After issues started using label `ai agent`,
`gh api …/issues?labels=ai agent` hung until the HTTP client timed out (unencoded space in the query
string). Maintain finished analysis and opened fix PRs, but raised **zero** issues. Direction: every
label filter must URL-encode; treat a hung list as a hard publish failure, not silence.
**Done in agent 0.6.0:** `_labelled_issues` percent-encodes label names (`ai%20agent`).

**14. Code findings keyed on summary thrash when wording drifts.** Two runs on the same path+symbol
with a rewritten summary opened a second issue (or failed to soft-update the first). Direction: path
findings carry a stable `slug` in the key; when capability+path+symbol matches exactly one open agent
issue, update that ticket even if the slug moved.
**Done in agent 0.6.0 (contract 6):** required `slug` on path subjects; soft-dedup in tracking.

## D. Open questions

1. Overlay size limit: the concrete value and the reaction on exceed — warning or startup error
   (section 10).

## E. Planned: trust surfaces
...
The knowledge side of the same plan is in Planned: trust surfaces below.

## E. Planned: design-faithful review
...
The knowledge side is in Planned: design-faithful below.

## E. Planned: deployment and infrastructure capabilities
...
The knowledge side of the same plan is in Planned: deployment below.
