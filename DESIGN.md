...
in the root `CHANGELOG.md` and, for judgement-focused notes, `knowledge/CHANGELOG.md`.
...
is not mistaken for a full sweep. The knowledge half is in Found in operation below.
...
`cleared_pin_target`. Remaining: truncate large tool stdout, run-level registry fact cache, cheaper
models for mechanical outdated, separate fixer token reserve.
...
of the same gap is in Found in operation below.
...
(item 3). The knowledge half is in Found in operation below.
...
is in Found in operation below.
...
treatment. Direction — tasks for the runner (knowledge half in Found in operation below):
