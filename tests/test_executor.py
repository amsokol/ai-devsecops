...
def test_an_analyst_result_is_kept_when_the_checkout_was_touched(
    ...
    assert executed[0].outcome.outcome is Outcome.CLEAN
    assert len(executed[0].attempts) == 1
    assert executed[0].attempts[0].rejected == ""
...
def test_a_fixer_that_edits_the_checkout_has_its_result_refused(
    library: Library, git_repo: Path, tmp_path: Path
) -> None:
    """A claimed fix that landed outside the worktree is not a fix on the branch."""
    ...
    backend = Straying(default=Scripted(result={"outcome": "fixed"}))
    task = PlannedTask(
        id="fix-x",
        ...
        scope=(),
        knowledge=("playbooks/maintain",),
    )
    toolkit = MagicMock()
    toolkit.tools.return_value = ()
    roster = Roster.of(backend)
    ...
    attempted = asyncio.run(
        run_attempts(
            ...
            budget=BUDGET,
            toolkit=toolkit,
            prompt_for=lambda _n, refused, path: f"retry={refused!r} → {path}",
            parse=lambda path: {"raw": path.read_text(encoding="utf-8")},
            checkout=Checkout.of(git_repo),
        )
    )

    assert attempted.parsed is None
    assert "checkout" in attempted.rejected
    assert [stray.path for stray in attempted.attempts[0].strayed] == ["README.md"]
    assert (git_repo / "README.md").read_text(encoding="utf-8") == "product\n"
    assert len(backend.briefs) == 2  # refused once, then retried
